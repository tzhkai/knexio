...
export default function NotFound() { return <Layout><SeoMeta title="Page not found" description="This page is not in the Workflow Library index." noIndex schema={({ origin, pageUrl }) => breadcrumbListSchema(origin, pageUrl, [{ label: "Home", href: "/" }, { label: "Page not found" }])} /><SiteBreadcrumb items={[{ label: "Home", href: "/" }, { label: "Page not found" }]} /><section className="not-found-wrap"><span className="eyebrow">Index mismatch</span><h1>This page is not in the library.</h1><p>The link may have moved, or the guide has not been filed here yet.</p><Link href="/guides" className="primary-button">Browse the guide library <ArrowUpRight size={17} /></Link></section></Layout>; }
