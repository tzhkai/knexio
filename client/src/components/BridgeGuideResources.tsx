...
export function buildResourceMatchSummary(count: number, query: string) {
  const label = count === 1 ? "1 related resource" : `${count} related resources`;
  const normalizedQuery = query.trim();
  return normalizedQuery ? `${label} match “${normalizedQuery}”` : `${label} available`;
}
...
export function isResourceSearchShortcut(event: Pick<KeyboardEvent, "key" | "metaKey" | "ctrlKey" | "altKey" | "shiftKey">) {
...
function HighlightedResourceText({ text, query }: { text: string; query: string }) {
  return <>{splitResourceHighlight(text, query).map((segment, index) => segment.isMatch ? <mark key={`${segment.text}-${index}`}>{segment.text}</mark> : <span key={`${segment.text}-${index}`}>{segment.text}</span>)}</>;
}
...
export default function BridgeGuideResources() {
  ...
  const matchSummary = buildResourceMatchSummary(resources.length, query);
  useEffect(() => {
    setIsFiltering(true);
  ...
  useEffect(() => {
    const focusSearch = (event: KeyboardEvent) => {
      if (!isResourceSearchShortcut(event) || isTextEntryTarget(event.target) || event.defaultPrevented) return;
