...
- [x] 保存检查点并交付结果
- [ ] 在可验证的真实 EEA/UK/Swiss 出口重新执行 Google CMP 首屏、拒绝、Manage options 与撤回四步；用户于 2026-08-26 暂缓此项，当前环境只能保留官方预览与非地区化观察证据
...
- [ ] Cloudflare 控制台在当前浏览器空白加载，尚待读取 `/games` 的时间范围、主机名、状态码和访问来源明细
...
- [ ] 审阅现有 404 自动补全、防抖状态、空状态和主题标签样式
- [ ] 增加自动补全计算期间的可访问加载动画提示
- [ ] 在无匹配结果时显示友好的可操作空状态提示
- [ ] 为 Browse by topic 标签增加尊重 reduced-motion 的平滑 hover 动效
- [ ] 补充加载、空状态和 hover 回归测试，完成构建及桌面/移动端验证
- [ ] 创建并核验包含本轮改动的 GitHub PR
- [ ] 按用户本次授权合并 PR 至 main，核验 Cloudflare 线上部署并保存证据

## AdSense 再申请前复审与修复

- [ ] 读取并核对现有 AdSense 站点审计技能、Google 官方要求和上次审计基线
- [ ] 对 knexio.xyz 当前线上页面、低价值内容风险、原创工作流价值、技术 SEO 与隐私边界执行只读审计
- [ ] 将审计结果按证据分为必须修复、建议修复、无需改动和无法验证，避免重复无效改动
- [ ] 实施已确认的高价值内容或技术修复，不虚构案例、评价、数据或作者资历
- [ ] 完成 Vitest、TypeScript、静态构建、关键页面和线上/预发布复核
- [ ] 形成提交前风险结论；不在用户明确确认前提交 AdSense 审核
...
- [ ] 完成优化后的线上只读复审；在此之前不提交 AdSense 审核
...
- [ ] 从最新 GitHub main 创建仅包含本次内容差异化优化的审阅分支
- [ ] 推送分支并创建 Pull Request，核验文件范围、测试状态和可合并性
- [ ] 保存 PR 审阅检查点；未经再次确认不合并、不部署
