...
- [x] 审计 24 个规范页面的层级、主题归属、核心入口和现有链接模块。
- [x] 设计首页、内容库、系列页、主题页和文章页之间的双向链接架构。
- [x] 制定自然锚文本、链接模块位置、数量边界与新内容发布规则。
- [x] 编写 sitemap、内部链接与 Search Console 联动的抓取加速路线。
- [x] 交付完整策略和可直接实施的页面改造清单。

## AdSense 基础代码接入评估

- [x] 核对 AdSense 基础代码、网站审核和广告展示的官方要求。
- [x] 审计当前 Cookie 同意机制、隐私披露和广告脚本加载状态。
- [x] 确定基础验证代码的接入时机及其合规加载条件。
- [x] 交付接入顺序、代码位置与上线前置条件。

## AdSense 隐私政策模板

- [x] 核对 AdSense、Google 数据使用与当前 Cookie 同意机制的必要披露。
- [x] 起草完整英文 Privacy Policy 模板并突出需替换字段。
- [x] 核对模板不错误宣称当前未启用或尚未部署的服务。
- [x] 交付上线前审核清单与法律审阅提示。

## 独立 Privacy Policy 页面集成

- [x] 审计现有 `/privacy/` 路由、页脚链接与模板替换字段。
...
- [x] 同步至 GitHub 与 Cloudflare Pages，并复核生产页面内容。

## AdSense 审核提交

- [x] 核对生产站点、AdSense 基础脚本、ads.txt、Privacy Policy 与 CMP 的提交前状态。
- [x] 在 AdSense 后台提交 knexio.xyz 进行审核。
- [x] 记录后台返回的提交状态、审核期间的站点维护要点与后续检查时间。

## 新站可见度与内容增长计划

- [x] 采集新站切换上线后至今的 Search Console 数据，并核对当前规范工作流 URL 的索引状态。
- [x] 分析当前数据，识别已收录页面、可见度信号与优先增长主题。
- [x] 制定免费或低成本的初期流量获取动作与渠道边界。
- [x] 制定四周内容更新计划、内部链接动作和复盘指标。
- [x] 交付数据摘要与可执行增长路线图。

## 首周内容发布与旧 URL 清理

- [x] 审计旧 URL 的 200 回退、静态路由和可安全实施的 404/410 或相关 301 映射。
...
- [x] 审计 Writing、Meetings、Planning 专题内容覆盖与质量空白。
...
## Search Console 实际查询词审计

- [x] 确认 knexio.xyz Search Console 资源、搜索类型和日期范围。
- [x] 读取实际查询词的点击、曝光、点击率和平均排名。
- [x] 区分新站数据与旧站历史数据，识别数据起始边界。
- [x] 分析查询词对应页面与内容机会。
- [x] 交付关键词现状、优先级和下一步建议。

## 新站数据与旧 URL 边界纠正

- [x] 核验新站实际上线日期，并只使用上线后的 Search Console 数据判断流量；用户确认新站于 2026-08-16 前后上线。
- [x] 检查代表性的旧站 URL 是否返回 200、301/308 或真实 404；抽查旧 URL 均为 404，新规范指南为 200。
...
- [x] 核对 GA 最近 28 天低跳出率页面及其访问量、参与度和事件数据；当前可访问资源不是 knexio.xyz，无法建立对应关系。
- [x] 分析这些页面的国家/地区、来源/媒介、设备、浏览器和访问时段；已核对资源摘要中的国家、来源和页面标题，未找到站点资源。
- [x] 区分站主调试、自动化流量与可能的真实用户访问，并记录证据强度；当前不能将扩展资源的 Singapore/Direct 数据判定为 knexio.xyz 用户。
- [x] 交付页面级访问来源判断和后续 GA 过滤建议；建议先配置/确认 knexio.xyz 专属 GA4 Measurement ID，再分析最近 28 天数据。

## 旧站页面恢复与迁移评估

- [x] 盘点旧站工具页、内容页、历史 URL 与可用备份，区分页面类型和业务依赖。
...
- [x] 为两个工具页加入按页面隔离的本地草稿保存、刷新恢复和清空逻辑。
- [x] 在两个工具页底部加入相关工具推荐模块和合理内链。
- [x] 为提示词统计报告与 Markdown 预览结果加入本地文件导出功能。
- [x] 补充本地存储与导出测试，并完成桌面/移动端回归验证。

## 工具页 Token、高亮与 SEO 内容增强

- [x] 为 AI Prompt Word Counter 添加透明标注的主流大模型 Token 数量估算。
- [x] 为 Markdown Preview 添加安全的代码块语法高亮和代码复制按钮。
- [x] 为两个工具增加一键复制结果到剪贴板及成功提示。
- [x] 在两个工具主体下方添加使用指南或 FAQ 说明，并完善必要的 SEO 结构化信息。
- [x] 补充测试并完成桌面/移动端回归验证。
...
- [x] 审计两个工具页和主要入口的移动端可用性、导航、状态提示和内容层级。
- [x] 实施一组高价值 UX、性能感知和 Workflow Library 内链优化。
- [x] 补充相关测试并完成桌面/移动端回归验证。

## 继续优化：可访问性与操作反馈

- [x] 审计工具页按钮、输入区、状态提示和移动端操作层级。
- [x] 实施一组高价值可访问性、键盘操作和成功/错误反馈优化。
- [x] 补充测试并完成桌面/移动端回归验证。

## AdSense 审核与 SEO 优先级优化

- [x] 审计隐私政策、Cookie 同意、关于我们、联系我们、服务条款和广告合规提示。
- [x] 审计全站标题、描述、canonical、robots、sitemap、结构化数据和索引阻断问题。
- [x] 检查原创内容深度、作者/编辑责任、页面主题一致性和工具页薄内容风险。
- [x] 强化四个 Workflow 集群的核心页、指南深度和内部链接结构。
- [x] 完成移动端、性能、404、死链和 AdSense 申请前清单验证；已验证移动端截图、生产构建、关键路由、静态 404、robots.txt 与 sitemap_index.xml。

## GDPR/CCPA Cookie 与 Sitemap 自动化

- [x] 审计并优化 Cookie 横幅的必要、分析和广告同意边界，以及撤回/修改入口。
- [x] 更新隐私政策与 Cookie 设置文案，明确当前未启用的广告处理范围。
- [x] 优化 sitemap 单一来源和自动生成流程，覆盖新增工具与文章路由。
- [x] 添加 sitemap 格式、规范 URL、robots 引用和新增路由自动收录校验。
- [x] 完成 Cookie 横幅、sitemap、构建和移动端回归测试。

## 最终 AdSense/SEO 质量冲刺

- [x] 执行最终的 AdSense 合规、技术 SEO、性能、可访问性和页面信任审计。
- [x] 修复高风险的合规文案、索引、链接、结构化数据或移动端问题。
- [x] 补强仍显薄弱的内容集群、工具页说明和申请前信任信号。
- [x] 完成生产构建、自动化测试、关键页面和移动端最终回归。
- [x] 形成 AdSense 提交前的明确行动清单和未决风险说明。

## AdSense 资料交叉审计

- [x] 读取并记录用户提供的六篇 AdSense 经验资料及可访问来源。
- [x] 将第三方经验与 Google 官方政策交叉核对，标记过时、不可验证或有风险的建议。
- [x] 把可迁移建议映射到 Knexio 当前审核状态、内容、技术 SEO、隐私和流量策略。
- [x] 交付资料审计报告与按优先级排列的执行清单。

## 第三方 AdSense 审计 Skill 安装

- [x] 审查 Skill 来源仓库、许可证、文件结构和潜在外部操作。
...
- [x] 在 EEA/UK/Swiss 预览或测试环境核验 Google CMP 同意、拒绝、管理和撤回流程。
- [x] 检查分析脚本真实加载状态，并据证据更新 Privacy 的提供方、用途、位置和保留期限。

## Google CMP 前台接入最终核验

- [x] 核对 Google Privacy & Messaging/CMP 集成规范与当前站点实现。
- [x] 检查线上 CMP、AdSense 脚本、同意状态和撤回入口的真实加载关系。
- [x] 修复前台 CMP 接入或隐私披露中的明确缺口，并补充自动化测试。
- [x] 完成构建、移动端和线上回归，保存新的可回退检查点。

## 发布后 CMP 与 Policy Center 核验

- [x] 检查发布版本的 CMP/AdSense 脚本和官方调试入口状态。
...
## 工具预设模板与 Writing 路径自测
- [x] 为 AI Prompt Word Counter 添加一键复制预设模板
- [x] 为 Markdown Preview 添加一键复制预设模板
- [x] 为 Writing & Updates 四步路径增加完成后的自测清单
- [x] 运行测试、类型检查、生产构建和移动端回归
- [x] 保存检查点并交付变更说明

## 模板个性化与自测反馈
- [x] 为两个工具页添加自定义模板输入与本地保存
- [x] 为两个工具页添加恢复默认模板功能
- [x] 为预设模板复制成功添加明显 Toast 动画提示
- [x] 为 Writing 自测清单添加可勾选状态、进度条与勾选动效
- [x] 运行测试、类型检查、生产构建和移动端回归
- [x] 保存检查点并交付变更说明

## 模板迁移与 Writing 成就反馈
- [x] 为两个工具页添加自定义模板 JSON 导出功能
- [x] 为两个工具页添加自定义模板 JSON 导入与无效文件提示
- [x] 为 Writing 自测清单添加带确认弹窗的重置所有进度按钮
- [x] 为 Writing 自测 100% 完成状态添加庆祝徽章与动效
- [x] 运行测试、类型检查、生产构建和移动端回归
- [x] 保存检查点并交付变更说明

## 下一轮实用性增强
- [x] 为模板导出增加工具名称、模板类型和可读文件名
- [x] 为 Writing 100% 完成状态增加可复制的完成摘要
- [x] 为完成摘要增加复制成功与失败反馈
- [x] 运行测试、类型检查、生产构建和移动端回归
- [x] 保存检查点并交付变更说明

## Writing 完成摘要保存与分享
- [x] 为 Writing 完成摘要添加一键打印功能
- [x] 为 Writing 完成摘要添加长图导出功能
- [x] 添加导出成功/失败反馈与移动端适配
- [x] 运行测试、类型检查、生产构建和移动端回归
- [x] 保存检查点并交付变更说明
...
- [x] 为长图保存操作增加最终效果预览弹窗
- [x] 为打印操作增加最终效果预览弹窗
- [x] 增加预览弹窗的取消、确认、无障碍和移动端适配
- [x] 运行测试、类型检查、生产构建和移动端回归
- [x] 保存检查点并交付变更说明

## Writing 摘要复制与 PNG 导出
- [x] 在预览弹窗增加一键复制摘要按钮
- [x] 为长图保存增加 PNG 导出选项
- [x] 增加 PNG 渲染失败和剪贴板失败反馈
- [x] 运行测试、类型检查、生产构建和移动端回归
- [x] 保存检查点并交付变更说明

## Writing 社交分享增强
- [x] 为 PNG 长图增加 Twitter/X、Instagram 等社交平台预设比例
- [x] 为摘要复制增加纯文本和带原文链接两种格式
- [x] 为 PNG 生成增加加载动画、成功提示和失败反馈
- [x] 运行测试、类型检查、生产构建和移动端回归
- [x] 保存检查点并交付变更说明
...
## Writing 分享配置增强
- [x] 为带链接摘要增加可选 UTM 参数配置
- [x] 增加 LinkedIn、Pinterest 和自定义宽高预设
- [x] 为 PNG 增加 1x、2x、3x 高清倍率导出
- [x] 增加参数校验、错误反馈和移动端适配
- [x] 运行测试、类型检查、生产构建和移动端回归
- [x] 保存检查点并交付变更说明

## AdSense Site Auditor 线上复审
- [x] 使用 AdSense Site Auditor Skill 复核 knexio.xyz 技术 SEO、内容质量、信任页面、广告合规、CMP 和发布商文件
- [x] 对比上次 73 项审计结果，识别新增、已修复和仍待验证风险
- [x] 形成分级整改建议与 AdSense 提交前检查清单
- [x] 保存审计报告并交付结果
...
- [x] 为 AI Prompt Word Counter 增加一键复制结果摘要功能
- [x] 为复制结果增加成功、失败和降级状态提示
- [x] 为 Meeting Agenda、Meeting Follow-up、Weekly Priorities、Decision Log 四篇指南增加响应式目录导航
- [x] 为新增交互与 TOC 增加回归测试并完成测试、构建和桌面/移动端视觉验证
- [x] 保存本轮检查点并交付结果


## 工具分享与指南阅读导航增强

- [x] 为 AI Prompt Word Counter 增加一键生成分享文本的分享按钮
- [x] 分享文本包含规范工具链接和当前统计结果，并提供成功/失败提示
- [x] 在 AI Prompt Word Counter 底部增加四篇刚更新指南的相关指南模块
- [x] 为指南 TOC 增加平滑滚动，并确认锚点滚动位置不被固定导航遮挡
- [x] 为指南页面增加右下角返回顶部悬浮按钮并适配移动端
- [x] 完成测试、构建、桌面/移动端视觉验证并保存检查点


## 社交分享、指南卡片与 TXT 导出

- [x] 为 AI Prompt Word Counter 增加 Twitter 和 LinkedIn 快捷分享按钮
- [x] 为社交分享链接自动添加来源、媒介和活动 UTM 参数
- [x] 为相关指南卡片增加摘要描述和轻量悬浮放大效果
- [x] 增加当前输入文本与统计结果的 TXT 导出功能
- [x] 完成分享链接、导出内容、测试、构建和响应式视觉验证，并保存检查点

## 导出范围、分享反馈与快速清空

- [x] 为 TXT 导出增加“仅统计结果”与“统计结果及完整提示词”的范围选择
- [x] 为 Twitter/X 与 LinkedIn 分享增加加载状态和成功触发提示
- [x] 在输入区增加带轻量确认提示的一键清空按钮
- [x] 为新增工具状态、导出范围和清空流程补充回归测试
- [x] 完成生产构建、桌面/移动端视觉验证并保存检查点

## 快捷键、时间戳导出与模型上下文参考

- [x] 为 Clear prompt 和 Export TXT 增加不干扰编辑的键盘快捷键与悬浮提示
- [x] 为 TXT 文件名和内容头部增加本地时间戳
- [x] 在输入区下方增加 GPT-4、Claude 3.5、Gemini 1.5 的上下文长度参考标签
- [x] 在当前选定模型接近规划上限时提供轻量视觉预警
- [x] 为快捷键、时间戳与参考数据增加回归测试，并完成构建和响应式验证

## 新站 Search Console 表现诊断

- [x] 重新建立 Search Console 访问路径，或明确需用户补充的最小后台数据
- [x] 确认新站切换日期、Search Console 资源和可用数据窗口
- [x] 分析新站期内点击、曝光、查询词、页面与索引覆盖状态
- [x] 核对近期核心页面的抓取和收录进度，区分正常等待与技术阻塞
- [x] 执行公开抓取与索引诊断备选方案
...
## 多专题顺序阅读路径与 Meetings 进度保存

- [x] 审阅 Research、Planning 集群的指南顺序、工具关联和当前主题页结构
- [x] 为 Research 与 Planning 建立任务型顺序阅读路径，并在合适节点链接相关工具
- [x] 为 Meetings 路径增加本地存储的完成状态、当前步骤和重置操作
- [x] 为 Meetings 路径增加轻量、可访问的步骤进度指示器
- [x] 为路径数据与持久化进度增加回归测试，完成构建和桌面/移动端视觉验证
- [x] 保存检查点并交付结果

## 统一专题路径进度、摘要与完成反馈

- [x] 审阅 Research、Planning 与 Meetings 路径的步骤数据和现有进度交互
- [x] 为 Research、Planning 与 Meetings 统一本地保存的完成状态、当前步骤和重置操作
- [x] 为三类路径增加完成摘要的一键复制功能和明确反馈
- [x] 为进度条增加平滑过渡，并在 100% 时提供尊重 reduced-motion 的轻量庆祝反馈
- [x] 为持久化、摘要和完成状态补充测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付结果

## 专题分享与下一路径推荐

- [x] 审阅四个专题路径的分享摘要实现和专题集群顺序
- [x] 为完成摘要加入纯链接与可选 UTM 分享格式，并提供清晰选择控件
- [x] 为四个专题增加自然的跨专题“下一条路径”推荐卡片
- [x] 补充分享格式和推荐映射的回归测试，完成构建与桌面/移动端验证
- [x] 保持 Search Console 仅观察已发现页面，不重复提交索引请求
- [x] 保存检查点并交付结果

## 下一路径统计、交互与 Research–Planning 过渡内容

- [x] 审阅 Cookie 同意、分析脚本、现有路径卡片和内容数据模型
- [x] 设计并实现仅在允许站点分析后触发的最小化路径卡片点击事件
- [x] 为路径卡片增加尊重 reduced-motion 的悬停与点击反馈
- [x] 撰写一篇连接 Research 与 Planning 的原创过渡指南，并接入专题、相关推荐与 sitemap
- [x] 补充同意门控、事件负载、内容路由和视觉交互测试
- [x] 完成生产构建及桌面/移动端验证
- [x] 保存检查点并交付结果

## 过渡指南模板、阅读进度与相关资源

- [x] 审阅指南详情页、现有下载组件和相关资源数据结构
- [x] 制作可下载、可编辑的优先级计划模板并接入过渡指南
- [x] 为过渡指南增加顶部无障碍阅读进度条与 reduced-motion 降级
- [x] 为过渡指南增加底部相关资源区块，推荐相关工具和阅读材料
- [x] 补充模板、进度和资源模块的回归测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付结果

## 模板格式、资源统计与核心长文阅读进度

- [x] 审阅优先级计划模板、资源卡片、现有分析事件和核心长篇指南范围
- [x] 为优先级计划模板增加可下载 Markdown 格式选项
- [x] 为相关资源卡片增加仅在允许站点分析后触发的最小化点击事件
- [x] 将顶部无障碍阅读进度条扩展到其他核心长篇指南
- [x] 补充下载、分析事件和长文范围回归测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付结果

## Markdown 模板复制、反馈与资源筛选

- [x] 审阅模板组件、现有 Toast/复制反馈模式和资源数据结构
- [x] 为 Markdown 模板增加一键复制与下载成功反馈
- [x] 为相关资源区块增加可访问的全部、工具和阅读筛选
- [x] 补充交互状态、筛选与回归测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付结果

## 优先级表格复制、资源搜索与筛选过渡

- [x] 审阅优先级表 Markdown 范围、资源筛选状态和过渡样式
- [x] 为 Markdown 模板增加仅复制优先级表格操作与状态反馈
- [x] 为相关资源增加本地关键词搜索及空状态
- [x] 为筛选结果增加尊重 reduced-motion 的轻量过渡
- [x] 补充复制范围、搜索筛选和过渡回归测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付结果

## 资源搜索快捷键、清除与 CSV 模板

- [x] 审阅资源搜索焦点管理、现有清除行为和优先级表导出列
- [x] 为相关资源搜索增加键盘快捷键与显式一键清除操作
- [x] 为优先级计划模板增加 RFC 4180 兼容的 CSV 下载选项
- [x] 补充快捷键、清除与 CSV 导出测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付结果

## 搜索高亮、推荐空状态与多行 CSV 导出

- [x] 审阅搜索结果文本、空状态推荐边界和 CSV 空白行结构
- [x] 为资源搜索结果增加安全的关键词高亮
- [x] 为未找到结果添加可操作的推荐资源区块
- [x] 为优先级计划 CSV 增加多行空白导出选项
- [x] 补充高亮、空状态与 CSV 行数回归测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付结果

## CSV 行数、搜索匹配数量与打印预览

- [x] 审阅模板打印结构、CSV 行数边界与搜索结果状态
- [x] 为多行 CSV 下载增加可访问的空白行数选择器
- [x] 为资源搜索增加动态匹配数量提示
- [x] 为优先级计划模板增加打印友好的浏览器预览与打印入口
- [x] 补充行数、数量和打印样式回归测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付结果

## 打印页眉、CSV 偏好与标题匹配搜索

- [x] 审阅打印预览、CSV 行数选择和资源搜索范围实现
- [x] 为打印预览增加自定义页眉输入与打印内容接入
- [x] 用本地存储记住用户的 CSV 空白行数选择
- [x] 为资源搜索增加仅匹配标题的可访问开关
- [x] 补充页眉、偏好和标题匹配回归测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付结果

## 打印日期、项目名称与标题匹配偏好

- [x] 审阅打印页眉、项目名称命名和标题匹配偏好状态
- [x] 在打印预览页眉右侧增加自动生成的当前日期
- [x] 为优先级计划增加项目名称输入，并用于导出文件名和打印标题
- [x] 用本地存储记住仅匹配标题开关状态
- [x] 补充日期、项目命名与偏好回归测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付结果

## AdSense 审核拒绝诊断

- [x] 查看 AdSense 网站状态、拒绝原因、Policy Center 与通知详情
- [x] 用独立线上审计复核内容原创性、页面实质性、导航信任与技术合规
...
- [x] 保存检查点并交付结果
- [x] 在可验证的真实 EEA/UK/Swiss 出口重新执行 Google CMP 首屏、拒绝、Manage options 与撤回四步；用户于 2026-08-26 明确暂缓，当前保留官方预览与非地区化观察证据，不再作为本轮阻塞项

## 整体 SEO 复核与审核等待期建设

- [x] 审计正式站点的可抓取性、规范化、站点地图、结构化数据、元数据、内链与内容架构
- [x] 复核四篇旗舰指南的内容独立性、来源边界、页面可读性、移动端呈现和 SEO 输出
- [x] 制定 2026-08-31 前可执行的高价值内容与交互功能优先清单，不提前重新提交 AdSense 审核
- [x] 保存整体 SEO 审计结论与后续实施路线

## 已确认的旗舰内容生产同步

- [x] 复核本地变更、测试结果、GitHub main 祖先关系与本次发布范围
- [x] 创建 GitHub 备份分支，并将已验证的旗舰内容和 SEO 审计记录同步到独立审阅分支
- [x] 核验 GitHub 审阅分支的提交、文件范围和与 main 的差异；不改动 Cloudflare 生产部署
- [x] 保存 GitHub 审阅分支同步记录并交付结果；不提交 AdSense 审核或新的索引请求

## 阅读交互增强与公开站点 SEO 复核

- [x] 审阅现有文章目录、相关阅读、阅读进度和本地阅读交互，确定不重复的高价值增强
- [x] 实现至少一项可访问、隐私友好的文章阅读或发现交互，并增加对应回归测试
...
- [x] 创建仅包含目录进度、透明推荐理由、回归测试和相关审计记录的 GitHub 审阅分支，不改动 main 或生产
- [x] 已获用户确认：创建并核对审阅分支指向 main 的 Pull Request，确认文件范围、测试结果和未合并边界；PR #3 为 OPEN / MERGEABLE / CLEAN
- [x] 保存审阅发布记录并请求用户确认是否合并；不自行合并或触发生产部署

## 已确认的 PR #3 合并与生产核验

- [x] 合并 GitHub PR #3 至 main，并确认 Cloudflare 自动部署触发
- [x] 等待并核验生产版目录当前位置、透明推荐理由、canonical、结构化数据和 sitemap 输出
- [x] 保存上线核验结果；不提交 AdSense 审核或新的索引请求

## 阅读进度与文章分享增强

- [x] 审阅现有文章进度条、分享入口、键盘反馈、移动端布局和隐私边界，确定不重复的增强范围
- [x] 实现阅读进度与社交分享的缺失体验增强，并添加回归测试
- [x] 完成测试、类型检查、静态构建与桌面/移动端验证
- [x] 保存审阅发布记录并请求用户确认是否合并；PR #4 已获确认并于 2026-08-26 合并

## 阅读进度与文章分享 GitHub 审阅发布

- [x] 从最新 GitHub main 创建仅含本轮进度条、分享、测试与验证记录的审阅分支
- [x] 创建并核验指向 main 的 Pull Request，确认文件范围、可合并状态和 main 未改动
- [x] 保存审阅发布记录并等待用户对合并的明确确认；PR #4 已获确认并于 2026-08-26 合并

## Cloudflare `/games` 分析记录核查

- [x] Cloudflare 控制台在当前浏览器空白加载，已记录无法读取 `/games` 的时间范围、主机名、状态码和访问来源明细；不影响本轮 AdSense 决策
- [x] 核对当前 `knexio.xyz/games` 响应、历史部署与路由残留，区分历史记录、爬虫、缓存和现行访问
- [x] 汇报证据与低风险处置建议；在用户恢复明确确认前保持 PR #4 未合并

## `/games` 重定向、404 恢复页与 PR #4 上线

- [x] 审阅现有 Cloudflare Pages 重定向配置、NotFound 路由与旧 URL 范围，限定 `/games` 及其子路径
- [x] 实现 `/games`、`/games/` 与 `/games/*` 到首页的永久重定向，并保留其他无关旧 URL 的 404 行为
- [x] 重新设计 404 页面，提供首页、内容库、指南和可理解的旧链接恢复入口
- [x] 为重定向范围、404 内容与关键恢复链接增加回归测试，并完成构建和桌面/移动端验证
- [x] 将本轮改动补充到 PR #4，复核可合并状态后按用户确认合并至 main；合并提交为 `0f01acb`
- [x] 核验 Cloudflare 自动部署后的 `/games` 重定向、404 页面、PR #4 阅读/分享功能与 canonical 输出

## 404 搜索、动态效果与最新指南

- [x] 审阅 404 页面、现有指南搜索能力、更新时间字段和可复用视觉元素
- [x] 为 404 页面增加可访问的站内指南搜索与匹配结果
- [x] 增加无第三方资源、尊重 reduced-motion 的轻量动态插画效果
- [x] 增加基于真实更新时间排序的 Latest guides 模块，不使用虚假热门或行为数据
- [x] 补充搜索、最新排序和动态偏好回归测试，完成构建与桌面/移动端验证
- [x] 保存检查点并交付本地验证结果；后续仅在用户授权后创建 PR #5，未合并或部署

## 404 搜索与最新指南 GitHub 审阅发布

- [x] 从最新 GitHub main 创建仅包含本轮 404 搜索、动态效果、最新指南、测试与任务记录的审阅分支
- [x] 创建并核验指向 main 的 Pull Request，确认文件范围、可合并状态与 main 未改动
- [x] 保存审阅发布记录并等待用户对合并的明确确认；PR #5 已获确认并于 2026-08-26 合并

## 404 搜索与最新指南生产上线

- [x] 在合并前复核 PR #5 的分支头、可合并状态和 main 基线
- [x] 按用户确认合并 PR #5 至 main，并记录合并提交 `8a3eca5`
- [x] 核验 Cloudflare 自动部署的动态 404、静态 404 搜索交接、Latest guides 与内容库查询结果
- [x] 保存生产核验证据并交付结果；未执行任何 AdSense、CMP 或 Search Console 操作

## 404 搜索建议、高亮与失效链接反馈

- [x] 审阅现有 404 搜索、指南数据、联系邮箱与静态 404 回退结构
- [x] 增加基于真实指南标题的搜索提示与可访问自动补全
- [x] 为 404 匹配结果增加安全的关键词高亮显示
- [x] 增加预填失效路径的无追踪“Report broken link”邮件入口，并同步静态 404 回退页
- [x] 补充自动补全、高亮和反馈链接回归测试，完成构建及桌面/移动端验证
- [x] 保存检查点并交付本地验证结果；后续仅在用户授权后创建 PR #6，未合并或部署

## 404 搜索建议与失效链接反馈 GitHub 审阅发布

- [x] 从最新 GitHub main 创建仅包含本轮自动补全、高亮、反馈入口、测试与任务记录的审阅分支
- [x] 创建并核验指向 main 的 Pull Request，确认文件范围、可合并状态与 main 未改动
- [x] 保存审阅发布记录并等待用户对合并的明确确认；PR #6 已于 2026-08-28 合并为 `fb880ee`，生产 404 已核验

## 404 搜索性能、主题入口与反馈统计

- [x] 审阅现有自动补全状态更新、真实主题字段、可选分析 API 与隐私说明边界
- [x] 为 404 自动补全增加可取消的 debounce，避免每次击键重复计算
- [x] 在 404 搜索框下增加基于真实主题的 Browse by topic 标签，不宣称热门度
- [x] 为 Report broken link 点击增加同意门控的最小化事件统计，未同意时不记录
- [x] 补充 debounce、主题标签与统计门控回归测试，完成构建和桌面/移动端验证
- [x] 保存检查点；未经用户另行确认不创建 GitHub PR、不合并、不部署

## 404 搜索加载、空状态与主题悬停

- [x] 审阅现有 404 自动补全、防抖状态、空状态和主题标签样式
- [x] 增加自动补全计算期间的可访问加载动画提示；补充旋转指示器并尊重 reduced-motion
- [x] 在无匹配结果时显示友好的可操作空状态提示
- [x] 为 Browse by topic 标签增加尊重 reduced-motion 的平滑 hover 动效
- [x] 补充加载、空状态和 hover 回归测试，完成构建及桌面/移动端验证；92 项测试与 TypeScript 检查通过
- [x] 创建并核验包含本轮改动的 GitHub PR；已纳入 PR #6
- [x] 按用户本次授权合并 PR 至 main，核验 Cloudflare 线上部署并保存证据；PR #6 已合并并核验

## AdSense 再申请前复审与修复

- [x] 读取并核对现有 AdSense 站点审计技能、Google 官方要求和上次审计基线
- [x] 对 knexio.xyz 当前线上页面、低价值内容风险、原创工作流价值、技术 SEO 与隐私边界执行只读审计
- [x] 将审计结果按证据分为必须修复、建议修复、无需改动和无法验证，避免重复无效改动
- [x] 实施已确认的高价值内容或技术修复，不虚构案例、评价、数据或作者资历；内容差异化修复已由 PR #7 发布
- [x] 完成 Vitest、TypeScript、静态构建、关键页面和线上/预发布复核
- [x] 形成提交前风险结论；当前不建议立即再次申请，且未提交审核

## AdSense 复审后的实质内容优化

- [x] 为四篇旗舰指南补充页面专属的编辑记录、来源边界、未验证事项与实质检查说明
- [x] 为相邻任务页增加输入/输出差异表和“何时不要用本页”说明，降低模板重复与低价值风险
- [x] 完成内容优化后的 Vitest、TypeScript、Cloudflare 构建及桌面/移动端复核
- [x] 完成优化后的线上只读复审；当前仍显示 Low-value content，未提交 AdSense 审核

## AdSense 内容差异化第二批

- [x] 识别剩余相似指南的真实输入、输出和不适用场景差异
- [x] 为剩余相似指南接入页面级差异化说明，避免虚构案例或结果
- [x] 完成第二批内容优化的测试、构建和桌面/移动端复核

## AdSense 内容优化审阅 PR

- [x] 从最新 GitHub main 创建仅包含本次内容差异化优化的审阅分支
- [x] 推送分支并创建 Pull Request，核验文件范围、测试状态和可合并性；PR #7 OPEN / MERGEABLE / CLEAN
- [x] 保存 PR 审阅检查点；PR #7 已获用户确认并合并，Cloudflare 部署工作流成功；尚未提交 AdSense 审核

## AdSense 优化页面内链与抓取审计（2026-09-02）

- [x] 审计 18 个 AdSense 优化页面的现有站内链接来源、锚文本和孤立风险；详见 INTERNAL-LINK-AUDIT-2026-09-03.md
- [x] 核对 sitemap.xml、sitemap_index.xml 与 robots.txt 对 Google 发现和抓取的影响，形成不改动线上配置的操作建议
- [x] 为尚未收录的 14 个优化页面制定分层内链增强计划，明确来源页、目标页、锚文本意图和验证步骤；P0/P1 方案已写入内链审计报告

## AdSense 再申请前复核（2026-09-02）

- [x] 结合最新线上部署、18 个优化页面的 GSC 状态和内容审计，复核 AdSense 页面质量风险
- [x] 对照 Google 官方政策检查内容原创性、导航、隐私/CMP、ads.txt、sitemap、robots 与站点可访问性
- [x] 判断当前是否适合再次申请，并列出必须先补齐与可等待观察的事项；当前不建议立即申请，本轮未提交审核

## AdSense 低价值内容二次修复与重新申请（2026-09-04）

- [x] 根据 18 个目标页面的 GSC 状态和内链审计，确定优先补强页面与差异化内容主题
- [x] 为优先页面增加原创的任务背景、可验证示例、边界说明和实质性跨专题内链，不虚构用户评价、结果或数据；新增 13 个页面级实践模块并接入正文/TOC
- [x] 同步真实修改页面的 sitemap lastmod，并完成内容、内链、SEO 与隐私边界回归测试；93 项测试、TypeScript、sitemap、Cloudflare 构建和桌面/移动端抽查通过
- [ ] 完成 GitHub/Cloudflare 发布后的线上页面、sitemap、robots、ads.txt 和关键路由核验
- [ ] 在后台复查 AdSense 网站状态与 Policy Center，并在满足前置条件后提交一次新的审核请求

## GitHub 自动发布与 AdSense 二次线上复查（2026-09-05）

- [ ] 将检查点 32182fa1 对应的页面价值增强改动推送到 GitHub，触发 Cloudflare 自动发布并核验部署结果
- [ ] 发布后复查核心指南正文、页面级实践模块、sitemap、robots、ads.txt 和关键路由
- [ ] 根据线上复查结果判断是否仍需追加实质性内容差异化优化；不以空泛字数或虚构案例替代价值提升

- [x] 修复本次内容更新后 sitemap-guides.xml 仍使用旧 lastmod 的元数据回归问题，并补充重复路由键测试；93 项测试、TypeScript 和 sitemap 生成通过
